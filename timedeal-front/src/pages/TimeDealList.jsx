...
import { PaymentModal } from './TimeDealDetail';
...
const TimeDealList = () => {
  ...
  const handleUrgentOpenPG = () => {
    if (!urgentPayment.method) return;
    setUrgentPayment(p => ({ ...p, showPayment: false, showPG: true }));
  };

  const handleUrgentPGComplete = async () => {
    const deal = urgentPayment.deal;
    setUrgentPayment(p => ({ ...p, showPG: false, processing: true, processingStep: 1 }));
    try {
      const { orderId } = await createOrder(deal.id);
      const pollOrder = async () => {
        const order = await getOrder(orderId);
        if (order.status === 'COMPLETED' || order.status === 'FAILED') {
          setUrgentPayment({ showPayment: false, showPG: false, processing: false, processingStep: 0, deal: null, method: null });
          navigate(`/order/${orderId}`, { state: { order, paymentMethod: urgentPayment.method } });
          return;
        }
        setUrgentPayment(p => ({ ...p, processingStep: order.status === 'STOCK_RESERVED' ? 2 : 1 }));
        setTimeout(pollOrder, 800);
      };
      setTimeout(() => { setUrgentPayment(p => ({ ...p, processingStep: 2 })); setTimeout(pollOrder, 800); }, 1000);
    } catch (err) {
      setUrgentPayment(p => ({ ...p, processing: false }));
    }
  };

  const handleUrgentPGCancel = () => {
    setUrgentPayment(p => ({ ...p, showPG: false, showPayment: true }));
  };
  ...
  return (
    <div className="min-h-screen bg-mesh">
                  ...
                  <div className="hidden lg:flex flex-col w-52 flex-shrink-0 bg-white rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div className="flex flex-col flex-1 p-4 justify-between">
                      ...
                      <div>
                        ...
                        <button
                          onClick={() => setUrgentPayment(p => ({ ...p, showPayment: true, deal: urgentDeal }))}
      ...
      {urgentPayment.showPayment && urgentPayment.deal && (
        <PaymentModal
          deal={urgentPayment.deal}
          paymentMethod={urgentPayment.method}
          setPaymentMethod={(m) => setUrgentPayment(p => ({ ...p, method: m }))}
          onClose={() => setUrgentPayment(p => ({ ...p, showPayment: false, deal: null, method: null }))}
          onNext={handleUrgentOpenPG}
        />
      )}
      {urgentPayment.showPG && urgentPayment.deal && (
        <PGSimulator
          deal={urgentPayment.deal}
          paymentMethod={urgentPayment.method}
          onComplete={handleUrgentPGComplete}
          onCancel={handleUrgentPGCancel}
        />
      )}
    </div>
  );
