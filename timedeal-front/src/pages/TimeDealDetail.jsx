...
const TimeDealDetail = () => {
  ...
  useEffect(() => {
    if (!deal || showPayment || processing || showPGSimulator) return;
    const interval = setInterval(fetchDeal, 5000);
    return () => clearInterval(interval);
  }, [deal, showPayment, processing, showPGSimulator]);
  ...
  const handlePGComplete = async () => {
    setShowPGSimulator(false);
    setProcessing(true);
    setProcessingStep(1);
    try {
      const { orderId } = await createOrder(deal.id);
      const pollOrder = async () => {
        const order = await getOrder(orderId);
        if (order.status === 'PENDING') setProcessingStep(1);
        else if (order.status === 'STOCK_RESERVED') setProcessingStep(2);
        else if (order.status === 'COMPLETED' || order.status === 'FAILED') {
          navigate(`/order/${orderId}`, { state: { order, paymentMethod } });
          return;
        }
        setTimeout(pollOrder, 800);
      };
      setTimeout(() => { setProcessingStep(2); setTimeout(pollOrder, 800); }, 1000);
    } catch (err) {
      setError(err.message);
      setProcessing(false);
    }
  ...
  const handlePGCancel = () => { setShowPGSimulator(false); setShowPayment(true); };
...
export const PaymentModal = ({ deal, paymentMethod, setPaymentMethod, onClose, onNext }) => {
  const paymentMethods = [
    ...
    },
    {
      ...
      desc: '간편결제',
      icon: (
        <svg viewBox="0 0 24 24" className="w-8 h-8" fill="#3A1D1D">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-t-3xl animate-slide-up max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 pb-8 overflow-y-auto">
          ...
          <div className="space-y-3 mb-6">
            {paymentMethods.map(method => (
              <button key={method.id} onClick={() => setPaymentMethod(method.id)}
                className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 transition ${paymentMethod === method.id ? 'border-brand-500 bg-brand-50' : 'border-brand-200 hover:border-brand-300'}`}>
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center overflow-hidden flex-shrink-0 ${method.iconBg || ''}`}>
                  {method.icon}
                </div>
                <div className="flex-1 text-left">
                  <p className="font-semibold text-brand-800">{method.name}</p>
                  <p className="text-sm text-brand-500">{method.desc}</p>
                </div>
          ...
          <button onClick={onNext} disabled={!paymentMethod}
            className={`w-full py-4 rounded-2xl font-bold text-lg transition ${paymentMethod ? 'bg-brand-500 text-white hover:bg-brand-400' : 'bg-brand-200 text-brand-400 cursor-not-allowed'}`}>
            결제하기
          </button>
        </div>
      </div>
...
const ProcessingScreen = ({ step, deal }) => {
  const steps = [
    ...
    { id: 3, label: '주문 확정', desc: '주문을 최종 확정하고 있어요', icon: '✅' },
  ];
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center p-8">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="w-20 h-20 bg-brand-100 rounded-full flex items-center justify-center mx-auto mb-4"><span className="text-3xl">💳</span></div>
          <p className="text-brand-500 font-semibold mb-1">결제 완료!</p>
          <p className="text-brand-600 text-sm">주문을 처리하고 있어요</p>
        </div>
        <div className="bg-brand-50 rounded-2xl p-4 mb-6">
        ...
        <div className="space-y-3">
          {steps.map((s) => {
            ...
            return (
              <div key={s.id} className={`flex items-center gap-4 p-4 rounded-2xl transition-all duration-500 ${isActive ? 'bg-brand-500/10 border-2 border-brand-500' : isDone ? 'bg-brand-100' : 'bg-brand-50 opacity-50'}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${isActive ? 'bg-brand-500 text-white animate-pulse' : isDone ? 'bg-brand-500 text-white' : 'bg-brand-200'}`}>
                  {isDone ? '✓' : s.icon}
                </div>
                <div className="flex-1">
                  <p className={`font-semibold text-sm ${isActive || isDone ? 'text-brand-500' : 'text-brand-400'}`}>{s.label}</p>
                  {isActive && <p className="text-xs text-brand-600">{s.desc}</p>}
                </div>
        ...
        <div className="mt-8 p-4 bg-brand-100 rounded-2xl">
          <p className="text-brand-600 text-xs text-center">⚠️ 재고 소진 시 결제 금액은 자동 환불됩니다</p>
        </div>
      </div>
    </div>
  );
